...
> **测试护栏：** 439 个离线测试全绿（无网络、无 LLM）· **文档：** [架构](docs/architecture.md) · [面试导航](docs/interview-notes.md)
...
| **测试护栏** | **439 个离线测试**：merger golden/parity、回测净值数学、供应商健康、接口路由、AST 依赖无环、图拓扑分解 | 以"冻结行为"为主，业务正确性由 [评估集](docs/interview-notes.md) 体系补充 |
...
```bash
venv/Scripts/python.exe -m pytest tests/ -q    # 预期：439 passed（全离线）
```
...
| 测试 | `venv/Scripts/python.exe -m pytest tests/ -q` | 439 离线用例 |
