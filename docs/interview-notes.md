...
>
> **重要声明：本轮没有进行真实 API Key 或端到端 LLM 运行。** 因此“框架已实现 / 离线验证”和“真实业务效果已验证”必须分开说。
...
目前可以证明的是架构边界、离线护栏和部分回测产物；本轮没有运行真实 LLM API，所以不会把框架就绪描述成端到端业务效果。
...
真实 smoke 对确认 provider、权限、配额和报告落盘很有价值，但它需要 API Key、成本预算和稳定运行时间。本轮的目标是面试作品化，不进行真实 API 测试，所以我会把它明确列为待验证工作，而不是伪造一条“已跑通”的证据链。

如果进入下一阶段，我会先用一个小规模真实 smoke 验证 `AnalysisRequest → AnalysisService → stream_analysis → report`，再考虑消融和评测。

### Q14：没有真实 API，如何证明 Agent 工程质量？
...
- 离线验证：453 个离线测试覆盖状态迁移、图流、依赖无环、供应商路由、评测数学和工具参数契约，全部不需要网络和 LLM；
- 业务证据：回测产物存在但带明确限制；真实模型评测需要单独预算，我明确标注为"未实跑"。

面试官真正想确认的是我**分得清这三层**，而不是把没有跑过的结果包装成已验证。离线工程能力恰恰是 Agent Runtime 岗位最看重的部分。
...
### Q20：453 个测试是不是说明系统已经可靠？

**回答：**

它说明很多接口、状态迁移、图流、供应商路由、解析器和回测数学有离线护栏，但测试数量不是业务有效性的充分条件。当前测试仍以结构和冻结行为为主，真实 provider、数据漂移和交易成本影响仍需要单独验证。

### Q21：真实 LLM 链路跑过吗？

**回答：**

本轮没有运行真实 API。当前可以确认 `AnalysisRequest`、`AnalysisService`、`stream_analysis()`、事件协议和报告落盘路径已经有代码和离线测试支撑；但我不会把这描述成真实 provider 端到端验证。若要完成这项验证，需要单独准备 API Key、预算、ticker、日期和可复现的运行报告。
...
- 小规模真实正确性评测（框架已就绪，`real_model_run` 明确为 false）；
- 小规模消融实验；
- 多窗口回测；
- 交易成本 `cost_bps` 敏感性；
- vendors 逐链路类型化错误；
...
建议顺序是：先真实 smoke（如果目标转为运行验证），再真实评测和消融，再扩展回测和成本；如果目标仍是面试包装，则先保持事实边界，继续完善 README、架构图和演示材料。
