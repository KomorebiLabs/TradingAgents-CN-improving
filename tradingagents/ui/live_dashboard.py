...
class LiveDashboard:
    ...
    def run(self, event_iterator, stats_callback=None):
        """Run live dashboard driven by an execution-event iterator.

        The dashboard never interprets iterator items: the producer
        (cli/analyze/run_impl._apply_event) already pushed every widget
        update through the public methods before yielding each event.
        This loop only drives the generator, polls the stats handler and
        refreshes the display — chunk interpretation belongs solely to
        ChunkEventTranslator (see tradingagents/application/events.py).

        Args:
            event_iterator: An iterable of AnalysisEvent objects.
        ...
        with _live_obj as live:
            # Force an immediate render of the initial layout before waiting for the first event.
